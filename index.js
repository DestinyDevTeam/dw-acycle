  ...
  try {
    if (process.env.ACTIVE == "TRUE") {
      for (let i = 0; i < auctionIds.length; i++) {
        ...
        // Querrying the config table
        config = await rpc.get_table_rows({
          json: true,
          code: process.env.CONTRACT_NAME,
          scope: process.env.CONTRACT_NAME,
          table: "configs",
        ...
        // Get the pool type
        pool_type = config.rows[0].pool_type;

        // STEP 1: Freeze the contract
        await setfreeze(auctionId, 3);
        await new Promise((resolve) =>
          setTimeout(resolve, process.env.WAIT_AUCT)
        );

        // STEP 2: Call recyclebyaid action
        if (pool_type == 0 || pool_type == 2) {
          await recyclebyaid(auctionId);
          await new Promise((resolve) =>
        ...
        await loopstakes(auctionId, auction_cycle_count);
        await new Promise((resolve) =>
          setTimeout(resolve, process.env.WAIT_STKE)
        );

        // STEP 4: Call stakegiveout action
        await stakegiveout(auctionId, auction_cycle_count);
        await new Promise((resolve) =>
          setTimeout(resolve, process.env.WAIT_OPEN)
        );

        // STEP 5: Unfreeze
        await setfreeze(auctionId, 0);
      }
    } else {
